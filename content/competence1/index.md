...
Cette compétence se centre sur la réalisation et le développement d'application.
...
- Une capacité d'analyse de pages web assez importantes
...
- Un historisation des valeurs d'une page pour pouvoir les comparer à travers le temps
...
tâche en attente (une tâche est une page à analyser suivant une liste de critères définie) et commencer l'analyse avant retourner ses
...
l'interface de l'outil, celle permettant la création de rapport et leur suivi. La deuxième est l'interface d'administration permettant
...
L'interface administrateur a d'abord été développée en utilisant VueJS aussi, cependant des suites de changement de technologies du
côté de l'API, j'ai pu me permettre d'exploiter l'interface d'administration de Django directement, elle permet énormément d'opération
sur les données de l'outil, mais aussi elle est dotée d'une flexibilité et d'un niveau de personnalisation assez conséquents.
...
assez complet et permet une grande flexibilité de développement, néanmoins il est par moment trop épuré, voire lacunaire, ce qui m'avait
amené à devoir passer du temps à développer des éléments qui seraient basiques et fondamentaux dans d'autres cadriciel. Une autre raison
...
abstraction dans les interactions la base de données, ce n'est pas pour autant un ORM, mais il s'en approche. Ce que gagne SQLAlchemy en
...
réutilisation de composantes, un ORM améliorant grandement la qualité de développement. En l'état, Django ne permet pas la création d'API
...
résultats bruts des analyses effectuées par les workers et elles sont enregistrées pour chaque type de tâches (critères personnalisés,
...
L'intérêt premier de modéliser les rapports de cette façon et de pouvoir les découper de manière atomique, avec les rapports pouvant se
...
abouti, mais aussi de pouvoir accumuler l'évolution de la valeur des différentes critères pour une même page, permettant l'historisation
...
l'application. L'application étant décomposée en plusieurs parties le déploiement n'a pas forcément était aisé, en premier lieu, j'ai
